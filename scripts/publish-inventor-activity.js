#!/usr/bin/env node
// Publishes the ExtractInventorBOM Activity to APS Design Automation.
...
const BUNDLE_NAME   = process.env.BUNDLE_NAME  || 'InventorBOMExtractor';
const ALIAS         = process.env.ALIAS        || 'prod';
const ACTIVITY_ID   = process.env.ACTIVITY_ID  || 'ExtractInventorBOM';
...
async function resolveEngineId(token) {
  ...
  if (invEngines.length === 0) throw new Error('No Inventor engines found.');
...
function buildActivityDef(engineId) {
  return {
    ...
    // InventorCoreConsole loads the IAM via /i and the bundle via /al.
    // Zipped assemblies are auto-extracted to the working dir by Inventor DA.
    commandLine: [
      `$(engine.path)\\InventorCoreConsole.exe /i "$(args[inputFile].path)" /al "$(appbundles[${BUNDLE_NAME}].path)"`,
    ],
    parameters: {
      inputFile: {
        verb:        'get',
...
(async () => {
  console.log('══════════════════════════════════════════════');
  console.log(' APS Activity Publisher — ExtractInventorBOM');
  console.log('══════════════════════════════════════════════');
