#!/usr/bin/env node
// Publishes (or updates) the InventorBOMExtractor AppBundle to APS Design Automation.
// Mirrors the proven AutoCAD appbundle publish script but targets the Inventor engine family.
const fs    = require('fs');
...
async function resolveEngineId(token) {
  if (ENGINE_VERSION_HINT) {
    const id = `Autodesk.Inventor+${ENGINE_VERSION_HINT}`;
  ...
  const invEngines = allEngines
    .filter(e => /^Autodesk\.Inventor\+\d{4}$/.test(e))
    .sort((a, b) => {
  ...
  console.log(`   Available Inventor engines: ${invEngines.join(', ')}`);
  if (invEngines.length === 0) throw new Error('No Inventor engines found in DA API response.');
  console.log(`   ✅ Selected engine: ${invEngines[0]}`);
  return invEngines[0];
}
...
async function createNewVersion(token, engineId) {
  ...
  }, JSON.stringify({ id: BUNDLE_NAME, engine: engineId, description: `${BUNDLE_NAME} — Inventor BOM extractor` }));
  ...
  if (res.status === 409) {
    ...
    res = await request({
      ...
      method:   'POST',
      headers:  daHeaders(token),
    }, JSON.stringify({ engine: engineId, description: `${BUNDLE_NAME} — Inventor BOM extractor` }));
  }
...
async function verifyAlias(token, expectedVersion) {
  ...
  if (res.body.version !== expectedVersion)
    throw new Error(`Alias mismatch: expected v${expectedVersion} but server has v${res.body.version}. Re-run to recover.`);
  console.log(`   ✅ Alias verified: ${BUNDLE_NAME}+${ALIAS} → v${res.body.version}`);
}
