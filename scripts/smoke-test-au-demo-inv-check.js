...
// The .iam to open inside the zip (DA does not preserve names; this is the entry path).
const PATH_IN_ZIP   = process.env.PATH_IN_ZIP || 'BrewMain-Drawing/brewing-main.iam';
// Optional: require a specific key in result.json (e.g. EXPECT_KEY=topLevelRows for the
// real BOM extractor). Unset = generic check (parses + not ok:false), which is what a
// freshly-scaffolded smoke-stub bundle produces.
...
(async () => {
  ...
  const wiBody = JSON.stringify({
    ...
    arguments: {
      inputFile:  { url: inputUrl,  verb: 'get', pathInZip: PATH_IN_ZIP },
  ...
  console.log('──────────────────────────────────────────');
  // Strict no-BOM gate: a BOM breaks strict JSON parsers, so reject it outright.
  if (out.text.charCodeAt(0) === 0xFEFF) throw new Error('result.json starts with a UTF-8 BOM. Write it with new UTF8Encoding(false).');
  let parsed;
  try { parsed = JSON.parse(out.text); } catch { throw new Error('result.json is not valid JSON.'); }
  // Generic success check: parsed object that does not carry an explicit failure marker.
  // A freshly-scaffolded smoke-stub bundle writes {"smoke":"ok", ...} and passes here.
  if (typeof parsed !== 'object' || parsed === null) throw new Error(`result.json did not parse to an object: ${out.text.slice(0, 200)}`);
  if (parsed.ok === false) throw new Error(`result.json reports ok:false: ${out.text.slice(0, 200)}`);
  if (EXPECT_KEY && !(EXPECT_KEY in parsed)) throw new Error(`result.json missing expected key '${EXPECT_KEY}': ${out.text.slice(0, 200)}`);

  console.log(`✅ Smoke test PASSED — status:success, no BOM, valid JSON${EXPECT_KEY ? `, has '${EXPECT_KEY}'` : ''}.`);
