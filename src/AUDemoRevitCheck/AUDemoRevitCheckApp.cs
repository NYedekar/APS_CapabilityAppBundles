...
//
//  The FullClassName in the .addin MUST be AUDemoRevitCheck.AUDemoRevitCheckApp exactly.
//
...
//
//  REPLACE: AUDemoRevitCheck, AUDemoRevitCheckApp, and the processing body.
// ════════════════════════════════════════════════════════════════════════════
using System;
using System.IO;
using System.Text;
...
namespace AUDemoRevitCheck
{
    ...
    public class AUDemoRevitCheckApp : IExternalDBApplication
    {
        ...
        private void OnDesignAutomationReady(object sender, DesignAutomationReadyEventArgs e)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[AUDemoRevitCheckApp] Working dir files: {string.Join(", ", System.Linq.Enumerable.Select(wdFiles, f => Path.GetFileName(f)))}");

                var doc = e.DesignAutomationData.RevitDoc
                    ?? throw new InvalidOperationException("Revit document could not be opened.");

                Console.WriteLine($"[AUDemoRevitCheckApp] Processing: {doc.Title}");
                ...
                //
                // using (var tx = new Transaction(doc, "AUDemoRevitCheckApp"))
                // {
                //     tx.Start();
                //
                ...
                Console.WriteLine("[AUDemoRevitCheckApp] Done — result.json written.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[AUDemoRevitCheckApp] ERROR: {ex.Message}");
